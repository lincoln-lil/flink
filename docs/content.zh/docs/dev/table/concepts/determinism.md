...
## 2. 批处理都是确定性的吗？
在经典的批处理场景，对于给定的有界数据集，重复执行同一查询会得到一致的结果，这是对确定性最直观的理解。但实际上，同一个查询在批处理上也并不总是能得到一致的结果，让我们来看两个查询示例：
...
```
当用户在 '2022-08-22 12:02:00' 时刻提交该查询时，查询返回了表中全部的 6 条数据, 一分钟后，也就是在 '2022-08-22 12:03:00' 时刻再次执行该查询时, 只返回了 3 条数据：
```gitexclude
+------+---------------------+------------+
|  uid |               cTime |        url |
+------+---------------------+------------+
...
在批处理中的行为实际是有差异的，让我们继续看一个查询示例：
```sql
SELECT CURRENT_TIMESTAMP, * FROM clicks;
```

可以看到 `CURRENT_TIMESTAMP` 在返回的记录上都是同一个值：
```gitexclude
+-------------------------+------+---------------------+------------+
...
+-------------------------+------+---------------------+------------+
```
这个差异是因为 Flink 继承了 Apache Calcite 对函数的定义，在确定性函数之外存在不确定函数（non-deterministic function）
和动态函数（dynamic function, 内置的动态函数主要是时间函数）两类，不确定函数会在记录级别执行（每条记录单独计算），而动态函数仅在查询执行时确定对应的值，
记录级别不再执行（不同时间执行得到不同的值，但同一次执行得到的值一致）。 内置函数的不确定性完整列表可以参见 [内置不确定函数]( TODO link )。
注意：这个细分差异仅在内置函数中存在，用户自定义函数中仅有 `isDeterministic` 接口来声明函数的确定性

## 3. 流上的确定性
流计算和批处理的核心区别在于数据的无界性，回顾 Flink SQL 对流计算的抽象：计算的对象是随时间变化的动态表（dynamic table），
计算过程被抽象为在[动态表上的连续查询（continuous query）]({{< ref "docs/dev/table/concepts/dynamic_tables" >}}#dynamic-tables-amp-continuous-queries)。
在前文提及的批查询中函数的确定性因素，其中的动态函数在流的连续查询上也就等效于不确定性函数（逻辑上每条基表记录的变更都会触发流查询被执行），除此之外，流上还有其他的不确定性因素吗？

### 3.1 流上的不确定性因素及其影响
流场景中，除了不确定函数，其他可能产生不确定性的因素主要有：
1. 无法提供确定性回溯读取数据的 Source 连接器实现
2. 基于 PROCTIME 的计算，包括 Lookup Join 访问存在更新的表
3. 基于 TTL 淘汰内部状态数据（对双流 join、无限流聚合计算结果的影响）


对于 Flink SQL 来说，所能提供的确定性局限于计算部分，因为它本身并不存储用户数据（这里需要区别流模式中管理的内部状态和计算时输入的用户数据），可能产生不确定性的因素主要有：
