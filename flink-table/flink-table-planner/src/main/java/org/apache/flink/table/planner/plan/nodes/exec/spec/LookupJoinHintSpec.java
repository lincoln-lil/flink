...
public class LookupJoinHintSpec {
    ...
    public LookupJoinHintSpec(
            @JsonProperty(FIELD_NAME_ASYNC) Boolean async,
            @JsonProperty(FIELD_NAME_ASYNC_OUTPUT_MODE)
            ...
            @JsonProperty(FIELD_NAME_RETRY_FIXED_DELAY) Duration retryFixedDelay,
            @JsonProperty(FIELD_NAME_RETRY_MAX_ATTEMPTS) int retryMaxAttempts) {
        this.async = Preconditions.checkNotNull(async);
        this.asyncOutputMode = Preconditions.checkNotNull(asyncOutputMode);
        Preconditions.checkArgument(asyncCapacity > 0);
        this.asyncCapacity = asyncCapacity;
        this.asyncTimeout = Preconditions.checkNotNull(asyncTimeout);
        this.retryPredicate = retryPredicate;
        this.retryStrategy = Preconditions.checkNotNull(retryStrategy);
        this.retryFixedDelay = Preconditions.checkNotNull(retryFixedDelay);
        Preconditions.checkArgument(retryMaxAttempts > 0);
