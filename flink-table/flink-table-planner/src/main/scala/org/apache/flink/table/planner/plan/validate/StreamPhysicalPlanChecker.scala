...
import org.apache.flink.table.planner.plan.nodes.physical.stream.{StreamPhysicalCalcBase, StreamPhysicalChangelogNormalize, StreamPhysicalCorrelateBase, StreamPhysicalDataStreamScan, StreamPhysicalDeduplicate, StreamPhysicalDropUpdateBefore, StreamPhysicalExpand, StreamPhysicalGroupAggregateBase, StreamPhysicalLegacyTableSourceScan, StreamPhysicalLookupJoin, StreamPhysicalRel, StreamPhysicalSink, StreamPhysicalTableSourceScan, StreamPhysicalWindowAggregateBase}
...
object StreamPhysicalPlanChecker {
  ...
  def validatePhysicalPlan(
    ...
    val validated = physicalRelNodes.map {
      root =>
        root match {
          case sink: StreamPhysicalSink =>
            ...
            if (inputInsertOnly(sink)) {
              ...
              planChecker.visit(
                sink.getInput.asInstanceOf[StreamPhysicalRel],
                false,
                ImmutableBitSet.of())
            } else {
              // update streaming
...
//          throw new UnsupportedOperationException("")
        ...
        case NonDeterministicUpdateHandling.RESOLVE_LOOKUP_JOIN_ONLY =>
          // TODO support later, will return true for later correction
          throw new UnsupportedOperationException("")
      }
    }
     ...
     * @param requireUpsertKey
     *   downstream operator requires upsert key from the given node
      ...
      rel match {
        case calc: StreamPhysicalCalcBase =>
          ...
          if (requireDeterminism.nonEmpty) {
            ...
            conv2inputs.foreach(bitsetBuilder.set)
            val requireInputDeterminism = bitsetBuilder.build()

            // visit children
            val newChildren = visitChildren(calc, requireUpsertKey, requireInputDeterminism)
            calc.copy(calc.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]
          } else {
            calc
          }
          ...
          if (requireDeterminism.nonEmpty) {
            ...
            if (isDeterministic(correlate.scan.getCall)) {
              ...
              if (fromLeft.nonEmpty) {
                fromLeft.foreach(index => bitsetBuilder.set(index))
              }
              val requireInputDeterminism = bitsetBuilder.build()
              // visit children
              val newChildren = visitChildren(correlate, requireUpsertKey, requireInputDeterminism)

              return correlate
                .copy(correlate.getTraitSet, newChildren)
        ...
        case lookupJoin: StreamPhysicalLookupJoin =>
          if (!inputInsertOnly(lookupJoin)) {
            lazy val leftFieldCnt = lookupJoin.input.getRowType.getFieldCount
        ...
        case groupAgg: StreamPhysicalGroupAggregateBase =>
          ...
          val newChildren = visitChildren(groupAgg, requireUpsertKey, requireChildDeterminism)
          groupAgg.copy(groupAgg.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]
        ...
        case normalize: StreamPhysicalChangelogNormalize =>
          ...
          val newChildren = visitChildren(normalize, requireUpsertKey, requireChildDeterminism)
          normalize.copy(normalize.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]
        ...
        case dedup: StreamPhysicalDeduplicate =>
          ...
          val newChildren = visitChildren(dedup, requireUpsertKey, requireChildDeterminism)
          dedup.copy(dedup.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]

        case dropUB: StreamPhysicalDropUpdateBefore =>
          val newChildren = visitChildren(dropUB, requireUpsertKey, requireDeterminism)
          dropUB.copy(dropUB.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]
        ...
        case expand: StreamPhysicalExpand =>
          ...
          val newChildren = visitChildren(expand, requireUpsertKey, requireDeterminism)
          expand.copy(expand.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]
        ...
        case windowAgg: StreamPhysicalWindowAggregateBase =>
          ...
          val newChildren = visitChildren(windowAgg, requireUpsertKey, requireChildDeterminism)
          windowAgg.copy(windowAgg.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]

        case

        case _ =>
          // visit child
//          val newChildren = visitChildren(rel, requireUpsertKey, requireDeterminism)
//          rel.copy(rel.getTraitSet, newChildren).asInstanceOf[StreamPhysicalRel]
          // do nothing
          rel
      }
    ...
    private def visitChildren(
      ...
      val newChildren = for (i <- 0 until parent.getInputs.size()) yield {
        ...
        visit(child, requireUpsertKey, requireDeterminism)
      }
      newChildren.toList
    }
  }
