...
class HBaseTableSource(
  ...
  def getNumberOfFields: Int = fieldsInfo.length + 1

  /** Returns the names of the table fields. */
  def getFieldsNames: Array[String] = rowKey._1 +: fieldsInfo.map(_._1)

  /** Returns the types of the table fields. */
  def getFieldTypes: Array[TypeInformation[_]] = rowKey._2 +: fieldsInfo.map(_._2)
